...
#include "OpenDoor_SpellAction.h"
#include "../Player Characters/CharacterBase.h"
#include <Runtime/Engine/Public/DrawDebugHelpers.h>

UOpenDoor_SpellAction::UOpenDoor_SpellAction() {
    actionCost = 10;
}
bool UOpenDoor_SpellAction::Action(AActor* caster, bool bIsEnvironment)
{
    ...
    UE_LOG(LogTemp, Error, TEXT("Implementing the open door MF !"));
		...
		if (actor->IsA<ADoor_Base>()) {
			if (Cast<ADoor_Base>(actor)) {
				if (Cast<ADoor_Base>(actor)->getIsOpen()) {
					UE_LOG(LogTemp, Error, TEXT("Door already opened"));
					return false;
				}
			}
